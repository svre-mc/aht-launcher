...
export async function installPhoenixAntiCheat({
  ...
  try {
    ...
    await writeDownloadedBinary(response, temporaryPath, release, onProgress, controller.signal);
    ...
    await atomicPhoenixJson(statePath, record);
    onProgress({ phase: 'Ready', completedBytes: release.size, totalBytes: release.size, percent: 100 });
    return { required: true, supported: true, installed: true, valid: true, state: 'ready', ...record, consented: true, binaryPath: finalPath };
  } catch (error) {
