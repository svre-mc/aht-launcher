...
// Public SPKI pin for the Worker signing key. This is deliberately not a
// credential; the server-only state-channel bearer token is supplied only by
// the dedicated server environment.
const LAUNCHER_ATTESTATION_PUBLIC_KEY_SHA256 = '8ac90ac7194d749980d45e2ad616ebed85f298511adb14d1d60e49f3fa6d20a8';
...
async function buildClientManifest(filePath, inspection, { packId, name, version, forceHashPaths = [] }) {
  const entryPaths = [];
...
async function writeZipWithInjectedFiles({ sourceZip, destZip, injections, removePaths = [] }) {
  await ensureDir(path.dirname(destZip));
  const zipIn = await openZipFile(sourceZip);
  const zipOut = new yazl.ZipFile();
  ...
  const normalizedRemovals = new Set(removePaths
    .map((entry) => normalizeRelPath(entry).toLowerCase())
    .filter(Boolean));
  ...
  const inputDone = new Promise((resolve, reject) => {
    zipIn.on('entry', (entry) => {
      ...
      if (normalizedRemovals.has(normalizeRelPath(entryName).toLowerCase())) {
        zipIn.readEntry();
        return;
      }
...
function serverLockConfig({ packId }) {
  return [
    ...
    '# Launcher policy is received over one authenticated, signed server-only channel.',
    'general {',
    `    S:attestationPublicKeySha256=${LAUNCHER_ATTESTATION_PUBLIC_KEY_SHA256}`,
    ...
    '    S:stateWebSocketUrl=wss://api.ahardtime.net/server/launcher-state',
    '    I:timeoutTicks=300',
...
async function buildFullClientRelease(options, sourceInspection) {
  ...
  if (fullClientZipInjections.length) {
    await writeZipWithInjectedFiles({
      ...
      removePaths: existingClientVersionLockPaths.map((entry) => rootPrefix ? `${rootPrefix}${entry}` : entry)
    });
  } else if (copyZip) {
  ...
  const clientManifest = await buildClientManifest(artifactPath, artifactInspection, {
    ...
    name,
    version,
    forceHashPaths: versionLockJarPath ? [clientVersionLockPath] : []
  });
  ...
  const latest = {
    ...
    serverLock: {
      ...
      replaced: replaceClientVersionLock
    },
    itemFireFix: {
  ...
  const report = {
    ...
    output: {
      ...
      clientVersionLockReplaced: replaceClientVersionLock,
...
export async function buildRelease(options) {
  ...
  let overrideSummary = summarizeOverrides(zip, overridesDir);
  ...
  if (zipInjections.length) {
    await writeZipWithInjectedFiles({
      ...
      removePaths: versionLockJarPath ? existingClientVersionLockPaths : []
    });
  } else if (copyZip) {
    await fs.copyFile(packZip, zipDest);
  }
  ...
  const artifactPath = await pathExists(zipDest) ? zipDest : packZip;
  overrideSummary = summarizeOverrides(new AdmZip(artifactPath), overridesDir);
  const stats = await fs.stat(artifactPath);
  const sha256 = await hashFile(artifactPath, 'sha256');
