...
assert(desktopMain.includes('function curseForgeMinecraftRootCandidates()') && desktopMain.includes("state: 'curseforge'") && desktopMain.includes('const curseForgeRoot = await firstRootOwnedMinecraftExecutableRoot(curseForgeMinecraftRootCandidates())') && playCurseForgePrioritySmoke.includes('Play did not prefer the CurseForge Minecraft launcher root') && playCurseForgePrioritySmoke.includes('legacy.json') && playCurseForgePrioritySmoke.includes("minecraftLauncherOpenState !== 'curseforge'"), 'Windows Play must prefer CurseForge minecraft\\Install when present and prepare legacy 1.12 assets in all launcher roots.');
...
assert(desktopMain.includes('WINDOWS_MINECRAFT_PACKAGE_FAMILY') && desktopMain.includes("path.join(env.LOCALAPPDATA, 'Packages', WINDOWS_MINECRAFT_PACKAGE_FAMILY)"), 'Windows Store fallback must verify the Minecraft package exists before reporting launch success.');
...
assert(desktopMain.includes("label: 'Store fallback'") && desktopMain.includes('cannot guarantee --workDir'), 'Windows Store fallback must be labeled as degraded instead of normal Ready.');
