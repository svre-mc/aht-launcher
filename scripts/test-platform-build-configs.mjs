...
assert(playServiceOutageSmoke.includes('/assets/1.12.json') && playServiceOutageSmoke.includes('Minecraft Launcher opened') && playServiceOutageSmoke.includes('Play should open Minecraft Launcher instead of blocking on Mojang asset-service preflight') && playServiceOutageSmoke.includes('AHT_TEST_SPAWN_DETACHED_CAPTURE_PATH') && playServiceOutageSmoke.includes('Minecraft Launcher was not opened during Mojang asset-service outage'), 'Minecraft service outage Play smoke must prove Play still opens Minecraft Launcher instead of blocking on Mojang asset preflight.');
...
assert(playAssetRepairSmoke.includes('AHT_TEST_MINECRAFT_ASSET_BASE_URL') && playAssetRepairSmoke.includes('asset-objects') && playAssetRepairSmoke.includes('assetObjectRequestCount !== 0') && playAssetRepairSmoke.includes('Play should not block on asset-object repair before opening Minecraft Launcher') && playAssetRepairSmoke.includes('AHT_TEST_SPAWN_DETACHED_CAPTURE_PATH') && playAssetRepairSmoke.includes("['--workDir', mcRoot]") && playAssetRepairSmoke.includes('desktopMinecraftLauncher') && !playAssetRepairSmoke.includes('root-owned Minecraft Launcher executable') && playAssetRepairSmoke.includes('windowsHide === true') && playAssetRepairSmoke.includes('DISABLE_RTSS_LAYER'), 'Minecraft asset Play smoke must prove Play opens a real Minecraft Launcher visibly with --workDir without blocking on asset-object downloads.');
...
assert((desktopMain.match(/ensureAssetObjects: true/g) || []).length >= 1 && (desktopMain.match(/verifyAssetHashes: true/g) || []).length >= 1 && desktopMain.includes('Play does not block on Mojang asset downloads'), 'Modpack Update finalization must repair missing/corrupt Minecraft asset objects while Play opens Minecraft Launcher without blocking on asset downloads.');
