...
assert(playStartSource.indexOf("'prepared-prerequisites'") < playStartSource.indexOf("'protected-client-integrity'") && playStartSource.indexOf("'protected-client-integrity'") < playStartSource.indexOf("'phoenix-anticheat'") && playStartSource.indexOf("'phoenix-anticheat'") < playStartSource.indexOf('const launcherOpening = openMinecraftLauncher') && playStartSource.indexOf('const launcherOpening = openMinecraftLauncher') < playStartSource.indexOf("'prepared-play-attestation'") && playStartSource.includes('managedFilesChecked: finalManagedIntegrity.managedFilesChecked') && playStartSource.includes('runtimeFilesChecked: 0') && preparedPlayIntegritySource.includes('verifyManagedIntegritySnapshot') && preparedPlayIntegritySource.includes('forceAll: !trusted.complete && !previousFileStates.length') && preparedPlayIntegritySource.includes('let previousFileStates = trusted.complete ? trusted.fileStates : []') && preparedPlayIntegritySource.includes('matchingManagedFileStates(prepared.previousManagedSnapshot, managedFiles)') && playStartSource.includes('if (prepared.minecraftProfile?.selectionPrepared !== true)') && playStartSource.includes('selectPreparedMinecraftLauncherProfile(prepared.minecraftProfile)') && playStartSource.includes('await persistPreparedLaunchEntry(key, prepared)') && !playStartSource.includes('verifyPreparedRuntimeSnapshot') && playStartSource.includes('proof = await refreshPreparedLauncherProof(key, prepared, nativeGuard);'), 'Play must verify the protected path set before Phoenix and handoff, hash only metadata changes after its signed baseline, and retain the fast saved launcher/runtime path.');
assert(socialRequestContextSource.includes("'.aht-launcher', 'social'") && preparedProofRefreshSource.includes('const inFlight = await current.proofRefreshInFlight;') && preparedProofRefreshSource.includes('inFlight?.payload?.nativeGuardKeyHash === nativeGuard.keyHash') && playStartSource.includes("proof?.nativeGuard?.protocol !== 'AHT-GUARD-1'"), 'Social activity and an unguarded in-flight refresh must never overwrite or satisfy a Play-scoped Phoenix proof.');
assert(
...
assert(developerOnlySourceFiles.length === 7, 'Regular player package developer-only source files must be declared.');
...
assert(packageScripts['test:phoenix-anticheat'] === 'node scripts/test-phoenix-anticheat-install.mjs' && verifyLocalScript.includes("['test:phoenix-anticheat']"), 'Local verification must cover separate Phoenix download, consent, hashing, tamper detection, and cleanup.');
