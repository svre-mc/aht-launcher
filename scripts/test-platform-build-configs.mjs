...
assert(packageScripts['test:play-curseforge-storage-root'] === 'node scripts/smoke-play-curseforge-priority.mjs --curseforge-storage-root' && verifyLocalScript.includes("['test:play-curseforge-storage-root']") && verifyInstalledScript.includes("['test:play-curseforge-storage-root']"), 'CurseForge storage-root Play smoke must be wired into local and installed-player verification.');
...
assert(desktopMain.includes('function curseForgeStorageMinecraftRootCandidates') && desktopMain.includes("path.win32.join(process.env.APPDATA, 'CurseForge', 'storage.json')") && minecraftLauncherRoutesSource.includes('curseForgeRootHints') && playCurseForgePrioritySmoke.includes('--curseforge-storage-root') && playCurseForgePrioritySmoke.includes("'minecraft-settings'"), 'Play must read CurseForge storage.json minecraftRoot values, pass them as route hints, and smoke-test custom storage roots outside hardcoded paths.');
