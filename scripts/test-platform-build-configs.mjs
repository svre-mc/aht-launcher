...
assert(packageScripts['test:play-curseforge-app-fallback'] === 'node scripts/smoke-play-curseforge-priority.mjs --no-desktop --curseforge-app-fallback' && verifyLocalScript.includes("['test:play-curseforge-app-fallback']") && verifyInstalledScript.includes("['test:play-curseforge-app-fallback']"), 'CurseForge app fallback Play smoke must be wired into local and installed-player verification.');
...
assert(desktopMain.includes('planWindowsMinecraftLauncherRoutes({') && desktopMain.includes('readWindowsLauncherShortcutTargets') && desktopMain.includes('shell.readShortcutLink') && desktopMain.includes('windowsDiscoveredLauncherPaths') && !desktopMain.includes('function addWindowsLauncherRoute') && minecraftLauncherRoutesSource.includes('export async function planWindowsMinecraftLauncherRoutes') && minecraftLauncherRoutesSource.includes('minecraftLauncherPaths = []') && minecraftLauncherRoutesSource.includes('curseForgeAppPaths = []') && minecraftLauncherRoutesSource.includes("'curseforge'") && minecraftLauncherRoutesSource.includes("kind: 'desktop'") && minecraftLauncherRoutesSource.includes("kind: 'store'") && minecraftLauncherRoutesSource.includes("kind: 'curseforge-app'") && minecraftLauncherRoutesTest.includes('curseforge-desktop-store') && minecraftLauncherRoutesTest.includes('curseforge-app-before-store') && minecraftLauncherRoutesTest.includes('shortcut-minecraft-launcher') && minecraftLauncherRoutesTest.includes('shortcut-minecraft-exe-launcher') && minecraftLauncherRoutesTest.includes('shortcut-curseforge-app') && minecraftLauncherRoutesTest.includes('curseforge-app-only') && minecraftLauncherRoutesTest.includes('missing-launcher') && minecraftLauncherRoutesTest.includes("assert.notEqual(planned[0].command, path.win32.join(curseForgeRoot, 'minecraft.exe'))") && playCurseForgePrioritySmoke.includes("const expectedOpenState = 'preferred'") && playCurseForgePrioritySmoke.includes('minecraftLauncherRouteKinds') && playCurseForgePrioritySmoke.includes('minecraftLauncherHasCurseForgeRoute') && playCurseForgePrioritySmoke.includes('forced-failure') && playCurseForgePrioritySmoke.includes('Fallback smoke did not open the normal Minecraft Launcher after the CurseForge-root route failed') && playCurseForgePrioritySmoke.includes('Play did not open CurseForge before the Store fallback') && playCurseForgePrioritySmoke.includes('desktopMinecraftLauncher') && playCurseForgePrioritySmoke.includes('legacy.json'), 'Windows Play must use a pure route planner with shortcut discovery, prepare CurseForge first without spawning the raw CurseForge minecraft.exe no-op path, then fall back through desktop/root/CurseForge app/Store routes.');
