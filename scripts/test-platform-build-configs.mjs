...
assert(smokeCloseDuringUpdate.includes("javaPath: fakeJavaPath") && smokeCloseDuringUpdate.includes("AHT_TEST_JAVA_RUNTIME_PROBE: 'release-file'") && smokeCloseDuringUpdate.includes("AHT_MINECRAFT_MAC_APP: process.platform === 'darwin' ? macMinecraftApp : ''") && smokeCloseDuringUpdate.includes('clearTimeout(timer);'), 'Close-during-update validation must isolate window lifecycle behavior from host prerequisites and clear completed CDP timeout handles.');
