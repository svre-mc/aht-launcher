...
assert(desktopMain.includes('backup_app="\\${target_app}.previous-update"'), 'macOS launcher update helper must preserve shell target_app expansion instead of evaluating it as a JavaScript template variable.');
...
assert(rendererHtml.includes('<body class="is-booting">') && rendererHtml.includes('id="startupLoader"') && rendererHtml.includes('class="app-frame" aria-hidden="true" inert') && rendererApp.includes('async function bootstrapLauncher()') && rendererApp.includes('window.aht.prepareStartup()') && rendererApp.includes('const startupTasks = Promise.allSettled([') && rendererApp.includes('STARTUP_QUICK_MAX_MS = 4_000') && rendererApp.includes('if (startupState.firstInitialization)') && rendererApp.includes('if (!currentStatus) renderInitialStatusError(error);') && rendererPolishCss.includes('body.is-booting .app-frame') && rendererPolishCss.includes('.money-loader-system'), 'Startup must remain fail-closed behind the opaque loading screen while its one startup preparation pass, status, legal, fonts, window load, and assets settle, with a hard whole-gate sub-five-second quick-path deadline.');
...
assert(!rendererHtml.includes('id="sidebarSwitchLoader"') && !rendererApp.includes('SIDEBAR_SWITCH_LOAD_HOLD_MS') && !rendererApp.includes('refreshPrepared(nextPack') && !rendererApp.includes('forcePreparation: nextPack') && !rendererPolishCss.includes('.sidebar-switch-loader'), 'Sidebar pack switches must never show a loading icon or rerun full launch preparation.');
...
assert(localChangesSource.includes("PLAYER_MUTABLE_MANAGED_ROOTS = new Set(['config'])") && localChangesSource.includes('verifyManagedIntegritySnapshot') && desktopMain.includes("LAUNCH_PREPARATION_MANAGED_POLICY = 'launch-critical-managed-files/v1'"), 'Quick startup must ignore player-owned config writes while re-hashing metadata-changed launch-critical files.');
