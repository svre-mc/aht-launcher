...
assert(playServiceOutageSmoke.includes('/assets/1.12.json') && playServiceOutageSmoke.includes('Unexpected end of JSON input') && playServiceOutageSmoke.includes('Minecraft services') && playServiceOutageSmoke.includes('did not produce service-focused wording') && playServiceOutageSmoke.includes('AHT_TEST_ERROR_REPORT_CAPTURE_PATH') && playServiceOutageSmoke.includes('AHT_TEST_SPAWN_DETACHED_CAPTURE_PATH') && playServiceOutageSmoke.includes('Minecraft Launcher was opened even though asset preparation failed') && playServiceOutageSmoke.includes('Copy full error details'), 'Minecraft service outage Play smoke must prove service wording, clickable diagnostics, no launcher spawn, and block raw JSON parse errors.');
