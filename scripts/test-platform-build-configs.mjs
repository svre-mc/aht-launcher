...
const windowsBuilderSource = readText(new URL('../build/electron-builder.windows.cjs', import.meta.url));
const packagedGuardVerifier = readText(new URL('../scripts/verify-packaged-native-guard.mjs', import.meta.url));
...
assert(configs.windows.nsis?.differentialPackage === false && configs.windows.nsis?.useZip === true, 'Windows installer must avoid the solid 7z overlay associated with the prior generic Boxter heuristic.');
assert(!JSON.stringify(configs.windows.extraResources || []).includes('native-guard'), 'Phoenix Anti-cheat must never be embedded as a launcher extraResource.');
assert(!commonBuilder.regularPlayerConfig({ productName: 'x', output: 'x', target: 'x' }).files.some((entry) => /native-guard/i.test(entry)), 'Phoenix Anti-cheat build files must remain outside app.asar.');
...
assert(
  wranglerConfig.includes('pattern = "api.ahardtime.net"')
    ...
    && desktopMain.includes("const branded = matches.find((url) => {")
    && desktopMain.includes("new URL(url).origin === 'https://api.ahardtime.net'")
    && desktopMain.includes('if (branded) return branded;')
    && desktopMain.includes("if (process.env.AHT_TEST_HOOKS === '1')")
    && desktopMain.includes("['127.0.0.1', 'localhost', '[::1]'].includes(parsed.hostname.toLowerCase())")
    && desktopMain.includes("return 'https://api.ahardtime.net';")
...
assert(rendererHtml.includes('id="playerSusTab"') && rendererHtml.includes('data-player-data-view="sus"') && rendererHtml.includes('<span>Detected</span><span>Player</span><span>Process</span><span>Exact finding</span><span>Launcher</span><span>Phoenix</span>') && rendererApp.includes('window.aht.devPhoenixDetections(payload)') && preloadScript.includes("ipcRenderer.invoke('dev:phoenixDetections'") && desktopMain.includes("ipcMain.handle('dev:phoenixDetections'") && workerSource.includes("'/admin/session-reports'"), 'Authenticated Developer Player Data must include a paginated Sus tab with exact, bounded Phoenix process and module evidence behind a neutral admin route.');
...
assert(rendererHtml.includes('id="testLauncherReinstallButton"') && rendererHtml.includes('Test Player Update') && rendererHtml.includes('id="launcherReinstallStatus"') && rendererHtml.includes('Nothing is uploaded or published.'), 'Developer Launcher Updates must expose a concise local player-update test control and safety status.');
...
assert(
  ...
  && desktopMain.includes("let transferMode = 'hard-link';")
  && desktopMain.includes('await physicalFs.link(selectedPath, copiedPath);')
  && desktopMain.includes("transferMode = 'copy';")
  && desktopMain.includes("const copiedSha256 = transferMode === 'hard-link'")
  && desktopMain.includes('setTimeout(() => app.quit(), 250);'),
  'Authenticated Developer Mode must create a strict one-shot same-version request with a same-volume hard-link/copy fallback, open the physically validated installed regular launcher with a sanitized environment, and quit only after prompt-ready acknowledgement.'
);
assert(
...
assert(rendererApp.includes('Ready to restart') && rendererApp.includes('Restart now') && rendererApp.includes('restartLauncherSelfUpdate') && rendererApp.includes('let launcherUpdateRequestPending = false;') && rendererApp.includes('if (!state.running && !launcherUpdateRequestPending)') && rendererHtml.includes('data-state="available"') && rendererHtml.includes('launcher-update-actions') && rendererHtml.includes('launcherUpdateLog" class="launcher-update-log" hidden'), 'Launcher self-update UI must keep polling through the IPC startup race, use the compact launcher-native state design, and expose Restart only after the complete payload is staged.');
assert(!rendererApp.includes('lastResult?.preparedRestart?.strategy') && !/function launcherUpdateStateForRenderer[\s\S]{0,2400}preparedRestart:/.test(desktopMain), 'Player-visible launcher update state must use safe capability booleans without exposing the prepared helper descriptor.');
assert(desktopMain.includes('const LAUNCHER_UPDATE_MONITOR_INTERVAL_MS = 10_000') && desktopMain.includes("mainWindow.webContents.send('launcher:update-available'") && desktopMain.includes('checkedAt: Date.now()') && preloadScript.includes("ipcRenderer.on('launcher:update-available'") && rendererApp.includes('onLauncherUpdateAvailable?.((update)') && rendererApp.includes('launcherUpdateCheckCanReplaceCurrent'), 'An already-open or backgrounded launcher must receive a newly published update from the main-process ten-second monitor without a stale startup response overwriting it.');
assert(
  desktopMain.includes('developerLocalOnly: true')
  && /function startLauncherUpdateMonitor\(\) \{[\s\S]{0,220}if \(isDeveloperMode\(\)\)/.test(desktopMain)
  && /async function checkLauncherUpdateQuietly\(\) \{\s+if \(bootDeveloperMode\) return null;/.test(rendererApp),
  'The local-only Developer Launcher must bypass public update fetches in status, the background monitor, focus/online checks, and Play checks.'
);
assert(desktopMain.includes("byteOperationProgress('Downloading launcher', fileName, progress, 0, 72)") && desktopMain.includes("byteOperationProgress('Verifying launcher', fileName, progress, 72, 12)") && desktopMain.includes('function setLauncherUpdateProgress') && launcherUpdateStagingSource.includes('const expected = await inspectLauncherZip(archivePath)') && launcherUpdateStagingSource.includes('total: expected.declaredBytes') && utilsSource.includes('const firstPositiveProgress = normalizedLoaded > 0 && lastReportedLoaded <= 0;'), 'Launcher update progress must use transferred, verified, and fixed-total extraction bytes, publish the first non-zero chunk immediately, and remain monotonic through staging.');
...
assert(desktopMain.includes('function currentProcessIsLauncherUpdateCandidate') && desktopMain.includes('if (currentProcessIsLauncherUpdateCandidate(pending)) return pending;'), 'The nonce-bound updated process must own candidate validation while starting so concurrent renderer hydration cannot discard the pending record after the staging directory has been swapped into place.');
...
assert(desktopMain.includes('waitForLauncherUpdateHelperStart') && desktopMain.includes('current handoff nonce') && desktopMain.includes('launcherUpdateHandoffCanPrime') && desktopMain.includes('primed: true') && desktopMain.includes('prePrimed'), 'Launcher update readiness must mean the nonce-bound handoff helper is already armed and waiting before the restart prompt appears.');
assert(desktopMain.includes('app.releaseSingleInstanceLock();') && desktopMain.includes('if (mainWindow && !mainWindow.isDestroyed()) mainWindow.destroy();') && desktopMain.includes('app.exit(0);'), 'A verified prepared update must release the old single-instance lock and terminate its Electron window/process directly so lock or graceful-close negotiation cannot delay the restart handoff.');
...
assert((launcherUpdateHelper.match(/\$null = Assert-StagedReceipt/g) || []).length === 1 && launcherUpdateHelper.includes('Assert-StagedSwapReadyForActivation $paths.StagingDir $targetRelativePath'), 'Windows must complete its expensive full-tree verification before Ready, then use only bounded activation checks after exit for a sub-second restart handoff.');
assert(
  /async function validateCompletedLauncherUpdateCandidate[\s\S]{0,5200}const criticalRelativePaths = \[[\s\S]{0,1200}physicalFs\.stat\(absolutePath\)[\s\S]{0,900}const validated = \{ treeSha256: receipt\.treeSha256 \}/.test(desktopMain)
  && !/async function validateCompletedLauncherUpdateCandidate[\s\S]{0,5200}validateStagedWindowsLauncherUpdate\(/.test(desktopMain),
  'The relaunched candidate must use bounded receipt and boot-file checks instead of repeating the pre-Restart full-tree or executable-version scan.'
);
assert(
  desktopMain.includes('const developerLocalFastPath = developerClientBypassAllowed();')
  && desktopMain.includes("process.platform === 'win32' && !developerLocalFastPath")
  && desktopMain.includes("'Developer startup checks only the saved Java and launcher paths; pack and runtime validation remain explicit tools.'")
  && rendererApp.includes('if (!bootDeveloperMode && !startupFirstInitialization)')
  && rendererApp.includes('if (!bootDeveloperMode) void loadLauncherSocialLinks({ forceRefresh: true })'),
  'The local-only Developer Launcher must reuse saved Java/profile paths, skip automatic runtime scans, and avoid background public-content refreshes.'
);
...
assert(launcherUpdateBootstrap.includes('Start-Process -FilePath $powerShell') && launcherUpdateBootstrap.includes('-WindowStyle Hidden -PassThru') && desktopMain.includes('launcher-update-bootstrap.ps1'), 'Windows restart must use a hidden bootstrap that leaves an independent helper alive after Electron exits.');
...
assert(desktopMain.includes("path.join(app.getPath('home'), 'Applications'") && desktopMain.includes('fallback_app') && desktopMain.includes('stage_for_target "$target_app"') && desktopMain.includes('requested_candidate="$requested_target.next-update"'), 'macOS launcher update must pre-stage beside its target and fall back to the user Applications folder before exposing Restart.');
assert(desktopMain.includes('expectedBundleVersion: launcherPackageVersionForRelease(latestVersion || \'\')') && desktopMain.includes('[ "$bundle_version" != "$expected_bundle_version" ]'), 'macOS update staging must accept the npm-safe bundle version for zero-padded public launcher releases.');
...
assert(rendererApp.includes('startupTaskTimings.readiness = {') && rendererApp.includes('launchPreparationState: String(currentStatus?.launchPreparationState || "missing")') && smokePlayerUpdatePlay.includes("const warmStartupTaskTimings = await evaluate(client, 'window.__ahtStartupTaskTimings')") && smokePlayerUpdatePlay.includes('...(warmStartupTaskTimings?.readiness || {})') && !smokePlayerUpdatePlay.includes("const warmStatus = await evaluate(client, `window.aht.getStatus('stable', { preferCache: true })`)"), 'Warm-start validation must consume the status already settled behind the loading screen instead of issuing a redundant post-reveal status IPC that can contend with background News refreshes.');
...
assert(rendererApp.includes('if (initialState.initialized && !initialState.firstInitialization)') && rendererApp.includes('preparation = await window.aht.prepareStartup();') && rendererApp.indexOf('preparation = await window.aht.prepareStartup();') < rendererApp.indexOf('newsStatusResults = await loadNewsStatusResults(true);') && desktopMain.includes("const usePreparedPrerequisites = prepared?.state === 'ready';") && desktopMain.includes('? prepared.launcherConfig') && desktopMain.includes('const identity = await identityForStatus(') && desktopMain.includes('usePreparedPrerequisites ? prepared : null,\n    allowProtectedStorage,\n    options'), 'Warm startup must hydrate its trusted prerequisite entries before status rendering and reuse their launcher route while refreshing the signed-in account instead of repeating launcher discovery.');
...
const preparedPlayIntegritySource = desktopMain.slice(desktopMain.indexOf('async function verifyPreparedClientIntegrityAtPlay'), desktopMain.indexOf('function preparedLauncherPathsForSnapshot'));
...
const phoenixMonitorSource = desktopMain.slice(desktopMain.indexOf('function phoenixDetectionFingerprint'), desktopMain.indexOf('function phoenixAntiCheatInstallDir'));
const socialRequestContextSource = desktopMain.slice(desktopMain.indexOf('async function socialRequestContext'), desktopMain.indexOf('async function launcherLegalStatus'));
const preparedProofRefreshSource = desktopMain.slice(desktopMain.indexOf('async function refreshPreparedLauncherProof'), desktopMain.indexOf('function scheduleLaunchPreparationProofRefresh'));
const persistedNativeGuardSource = launcherProofSource.slice(launcherProofSource.indexOf('const persistedNativeGuard'), launcherProofSource.indexOf('const fileProof'));
const phoenixConsentMarkup = rendererHtml.slice(rendererHtml.indexOf('id="phoenixAntiCheatOverlay"'), rendererHtml.indexOf('id="phoenixAntiCheatInstallButton"'));
const phoenixConsentCss = rendererCss.slice(rendererCss.indexOf('.phoenix-anticheat-overlay'), rendererCss.indexOf('.repair-prompt-dialog h2,'));
assert(desktopMain.includes("STARTUP_PREREQUISITE_POLICY = 'java8-launcher-paths-and-play-integrity/v6'") && prerequisitePreparationSource.includes('preparedLauncherRouteAvailable') && prerequisitePreparationSource.includes('preparedJava8RuntimeAvailable') && prerequisitePreparationSource.includes('launcherProof: null') && prerequisitePreparationSource.includes('proofPreparedThisSession: false') && !prerequisitePreparationSource.includes('scanCurrentManagedIntegrity') && !prerequisitePreparationSource.includes('scanPlayIntegrity') && !prerequisitePreparationSource.includes('verifyManagedIntegritySnapshot') && !prerequisitePreparationSource.includes('verifyPreparedRuntimeSnapshot') && !prerequisitePreparationSource.includes('createLaunchPreparationMutationMonitor') && !prerequisitePreparationSource.includes('armLaunchPreparationWatcher') && !startupPreparationSource.includes('performLaunchPreparation') && !startupPreparationSource.includes('hydrateLaunchPreparationFromSnapshot') && persistedPreparationSource.includes('launcherPaths: preparedLauncherPathsForSnapshot') && persistedPreparationSource.includes('cachedEntry.managedFiles = managedSnapshot.managedFiles') && persistedPreparationSource.includes('cachedEntry.managedFileStates = managedSnapshot.fileStates') && !persistedPreparationSource.includes('runtimeFiles:'), 'Startup must preserve quick launcher-path caching, persist the signed protected-file baseline, and keep content hashing out of routine startup.');
assert(prerequisitePreparationSource.includes('const cachedJava8Available = await preparedJava8RuntimeAvailable(java8Runtime);') && prerequisitePreparationSource.includes('if (!cachedJava8Available)') && prerequisitePreparationSource.includes("const validateMinecraftRuntime = process.platform === 'win32' && !developerLocalFastPath") && prerequisitePreparationSource.includes('&& (!reusable || !cachedRuntimeMatches || !minecraftProfile?.loaderInstalled)') && prerequisitePreparationSource.includes('preparedRuntimeMatchesInstalled(reusable, installed)') && prerequisitePreparationSource.includes('if (validateMinecraftRuntime)') && prerequisitePreparationSource.includes('without rescanning game files'), 'A valid signed warm-start snapshot must only check the saved Java and launcher executable paths; unrelated pack-version changes must not force Minecraft runtime hashing.');
assert(playStartSource.indexOf("'prepared-prerequisites'") < playStartSource.indexOf("'protected-client-integrity'") && playStartSource.indexOf("'protected-client-integrity'") < playStartSource.indexOf("'phoenix-anticheat'") && playStartSource.indexOf("'phoenix-anticheat'") < playStartSource.indexOf("'prepared-play-attestation'") && playStartSource.indexOf("'prepared-play-attestation'") < playStartSource.indexOf('const launcherOpening = openMinecraftLauncher') && playStartSource.includes('managedFilesChecked: finalManagedIntegrity.managedFilesChecked') && playStartSource.includes('runtimeFilesChecked: 0') && preparedPlayIntegritySource.includes('verifyManagedIntegritySnapshot') && preparedPlayIntegritySource.includes('forceAll: !trusted.complete && !previousFileStates.length') && preparedPlayIntegritySource.includes('let previousFileStates = trusted.complete ? trusted.fileStates : []') && preparedPlayIntegritySource.includes('matchingManagedFileStates(prepared.previousManagedSnapshot, managedFiles)') && playStartSource.includes('if (prepared.minecraftProfile?.selectionPrepared !== true)') && playStartSource.includes('selectPreparedMinecraftLauncherProfile(prepared.minecraftProfile)') && playStartSource.includes('await persistPreparedLaunchEntry(key, prepared)') && !playStartSource.includes('verifyPreparedRuntimeSnapshot') && playStartSource.includes('proof = await refreshPreparedLauncherProof(key, prepared, nativeGuard, { allowInteractiveRecovery: true });'), 'Play must verify protected files, Phoenix and authorization before handoff, hash only metadata changes after its signed baseline, and retain the fast saved launcher/runtime path.');
assert(socialRequestContextSource.includes("'.aht-launcher', 'social'") && preparedProofRefreshSource.includes('const inFlight = await current.proofRefreshInFlight.catch(') && preparedProofRefreshSource.includes('inFlight?.payload?.nativeGuardKeyHash === nativeGuard.keyHash') && playStartSource.includes("proof?.nativeGuard?.protocol !== 'AHT-GUARD-1'"), 'Social activity and an unguarded in-flight refresh must never overwrite or satisfy a Play-scoped Phoenix proof.');
assert(
  rendererHtml.includes('id="phoenixAntiCheatOverlay"')
  && rendererHtml.includes('class="phoenix-anticheat-badge"')
  && rendererHtml.includes('data-phoenix-badge-animation')
  && !rendererHtml.includes('class="phoenix-anticheat-card-surface"')
  && !rendererHtml.includes('class="phoenix-anticheat-card-cut"')
  && !rendererHtml.includes('Required for Windows Play')
  && rendererHtml.includes('</span>Install</button>')
  && rendererHtml.includes('>Cancel</button>')
  && rendererHtml.includes('security checks needed')
  && rendererHtml.includes('Terms of Service and Privacy Policy')
  && rendererCss.includes('font-family: "AHT Source Serif"')
  && rendererApp.includes('ensurePhoenixAntiCheatBeforePlay')
  && preloadScript.includes("ipcRenderer.invoke('anticheat:install')")
  && desktopMain.includes("ipcMain.handle('anticheat:install'"),
  'First Windows Play must retain concise, implementation-neutral Phoenix consent and its native installation path.'
);
assert(
  rendererCss.includes('@keyframes phoenix-badge-ring-spin')
  && rendererCss.includes('@keyframes phoenix-badge-sheen')
  && rendererCss.includes('background: conic-gradient(from 214deg, transparent 0deg 258deg')
  && rendererCss.includes('background: conic-gradient(from 34deg, transparent 0deg 270deg')
  && rendererCss.includes('drop-shadow(0 0 3px rgba(255, 229, 172, 0.56))')
  && rendererCss.includes('drop-shadow(0 0 4px rgba(255, 225, 164, 0.32))')
  && rendererCss.includes('width: 35%;')
  && rendererCss.includes('rgba(255, 246, 219, 0.14), rgba(255, 219, 144, 0.24)')
  && rendererCss.includes('mix-blend-mode: screen;')
  && rendererCss.includes('animation: phoenix-badge-sheen 5.4s')
  && rendererCss.includes('30% { opacity: 0.8; }')
  && (phoenixConsentCss.match(/content: none;/g) || []).length >= 3
  && !phoenixConsentCss.includes('repeating-linear-gradient')
  && !phoenixConsentCss.includes('0 0 32px rgba(223, 179, 106')
  && !phoenixConsentCss.includes('0 0 48px rgba(223, 129, 63')
  && phoenixConsentCss.includes('.phoenix-anticheat-visual::before')
  && phoenixConsentCss.includes('radial-gradient(ellipse 66% 62% at 37% 30%, rgba(235, 194, 119, 0.15)')
  && phoenixConsentCss.includes('.phoenix-anticheat-mark::before')
  && phoenixConsentCss.includes('radial-gradient(ellipse, rgba(218, 132, 66, 0.18)')
  && phoenixConsentCss.includes('radial-gradient(ellipse 72% 52% at 48% 39%, rgba(225, 238, 244, 0.025)')
  && phoenixConsentCss.includes('radial-gradient(ellipse 115% 76% at -18% 46%, rgba(223, 179, 106, 0.045)')
  && phoenixConsentCss.includes('linear-gradient(180deg, #090c0c 0%, #060808 52%, #030404 100%)'),
  'Phoenix must preserve the approved rotating lights, shine sweep, and separate asymmetric badge backlights.'
);
assert(
  !phoenixConsentCss.includes('.phoenix-anticheat-card-cut')
  && !phoenixConsentCss.includes('.phoenix-anticheat-card-surface')
  && !phoenixConsentCss.includes('@keyframes phoenix-card-slice')
  && !phoenixConsentCss.includes('@keyframes phoenix-card-surface')
  && phoenixConsentCss.includes('radial-gradient(ellipse 72% 52% at 48% 39%, rgba(225, 238, 244, 0.025)')
  && phoenixConsentCss.includes('linear-gradient(154deg, #0b0f0f 0%, #050707 48%, #020303 100%)'),
  'Phoenix brand panel must remain uninterrupted, with no incision markup, cutting overlay, or cut animation.'
);
assert(nativeGuardSource.includes("redirect: 'error'") && nativeGuardSource.includes('sha256') && nativeGuardSource.includes('PHOENIX_ANTI_CHEAT_CONSENT_VERSION') && nativeGuardSource.includes('launcherPid') && nativeGuardNotice.includes('does not install a service') && nativeGuardNotice.includes('does not run at system startup') && nativeGuardNotice.includes('does not use a kernel driver'), 'Phoenix Anti-cheat must be hash-pinned, consent-versioned, launch-bound, and explicitly free of service, startup, or Ring-0 persistence.');
...
assert(rendererApp.includes('SIDEBAR_SWITCH_EXIT_DELAY_MS = 50') && rendererApp.includes('SIDEBAR_SWITCH_EXIT_MS = 180') && rendererApp.includes('SIDEBAR_SWITCH_ENTER_MS = 330') && rendererApp.includes('animateSidebarOpacity(sourceView, 1, 0, SIDEBAR_SWITCH_EXIT_MS, "ease-in")') && rendererApp.includes('animateSidebarOpacity(incomingView, 0, 1, SIDEBAR_SWITCH_ENTER_MS, "ease-out")') && rendererApp.includes('window.setInterval(tick, 16)') && rendererApp.includes('packStatusCache.get(nextPack)') && rendererApp.includes('window.aht.selectPreparedPlay(nextPack)') && rendererPolishCss.includes('.workspace > .view.sidebar-view-leaving') && rendererPolishCss.includes('.workspace > .view.sidebar-view-entering.sidebar-view-entering-active'), 'Sidebar switches must preserve the measured BSG opacity transition on the launcher monotonic clock, independent of compositor animation progress, while using already-prepared status and only the lightweight profile-selection write.');
...
assert(packageScripts['test:phoenix-anticheat'] === 'node scripts/test-phoenix-anticheat-install.mjs && node scripts/test-phoenix-recovery.mjs' && verifyLocalScript.includes("['test:phoenix-anticheat']"), 'Local verification must cover separate Phoenix download, consent, hashing, tamper detection, recovery, and cleanup.');
...
assert(checkProductionReadiness.includes('live Worker player-data API is current') && checkProductionReadiness.includes('/admin/player-records') && checkProductionReadiness.includes('/admin/launcher-updates') && checkProductionReadiness.includes('/admin/session-reports') && checkProductionReadiness.includes("['api', 'session', 'report']"), 'Production readiness must fail when the deployed Worker is missing the current player-data and session-report routes.');
...
assert(checkProductionReadiness.includes('Import-Module Microsoft.PowerShell.Security -ErrorAction Stop') && checkProductionReadiness.includes("path.join(process.env.ProgramFiles || 'C:\\\\Program Files', 'PowerShell', '7', 'pwsh.exe')"), 'Production readiness must prefer modern PowerShell and fail closed when the Authenticode module cannot load.');
assert(!checkProductionReadiness.includes('AHT-Launcher-Windows-10-11-${version}.exe.blockmap'), 'Production readiness must not require an NSIS blockmap because launcher updates use the separately staged ZIP artifact.');
assert(checkProductionReadiness.includes('ignoredSourcePaths') && checkProductionReadiness.includes('gitIgnoredRelativePaths(sourceCandidates)'), 'Production readiness artifact freshness must ignore generated build inputs.');
...
assert(launcherUpdateManifestSource.includes("'macos-universal', 'ubuntu-x64-appimage'") && launcherUpdateManifestSource.includes('REQUIRED_STAGED_WINDOWS_KEYS') && launcherUpdateManifestSource.includes('REQUIRED_STAGED_LINUX_KEYS') && launcherUpdateManifestSource.includes('manual downloads contain unexpected keys') && launcherUpdateManifestSource.includes("isPortableDownload ? 'appimage' : 'deb'") && launcherUpdateManifestSource.includes('must include /S silent install args') && launcherUpdateManifestSource.includes('validatePhoenixAntiCheat') && launcherUpdateManifestSource.includes('launcher/anticheat/win32-x64/'), 'Launcher update manifest validator must lock the three public installers, compatibility update formats, and the separately hash-pinned Phoenix release.');
...
assert(launcherUpdateManifestSource.includes('assertLauncherReleaseAdvance') && launcherUpdateManifestSource.includes('already published') && launcherUpdateManifestSource.includes('KNOWN_LEGACY_DOWNLOAD_KEYS') && launcherReleaseImmutabilityCheck.includes('Could not prove launcher release immutability') && launcherReleaseImmutabilityCheck.includes('allowKnownLegacyDownloadKeys: true'), 'Launcher publication must compare known legacy live manifests while keeping new candidates strict and must fail closed before replacing an existing or newer published version.');
...
assert(
  desktopMain.includes("STARTUP_PREREQUISITE_POLICY = 'java8-launcher-paths-and-play-integrity/v6'")
  && desktopMain.includes("'Reuse initialized Java and launcher paths'")
  && desktopMain.includes("'Verify protected client files before Play'")
  && desktopMain.includes('managedFilesChecked: finalManagedIntegrity.managedFilesChecked')
  ...
  && smokePlayIntegrityGate.includes('Play did not issue exactly one fresh one-time Worker proof')
  ...
  'Initialized Play must reuse saved Java/launcher prerequisites, reject changed protected files before handoff, open within 500 ms on an unchanged baseline even while a cold proof request is pending, verify that proof before confirmed handoff, and tolerate Windows foreground-focus denial for a visible launcher.'
);
assert(
...
assert(localChangesSource.includes("PLAYER_MUTABLE_MANAGED_ROOTS = new Set(['config'])") && localChangesSource.includes('verifyManagedIntegritySnapshot') && desktopMain.includes("ipcMain.handle('files:scan'") && desktopMain.includes('scanCurrentManagedIntegrity(config)') && desktopMain.includes("writeIntegrityState(config, integrity, 'scan')") && preparedPlayIntegritySource.includes('verifyManagedIntegritySnapshot'), 'Managed integrity verification must cover explicit Scan, Update, and Repair plus the metadata-first protected-file Play gate, while player configs stay mutable.');
...
const identityForStatusSource = desktopMain.slice(desktopMain.indexOf('async function identityForStatus'), desktopMain.indexOf('function identityForRenderer'));
...
assert(
  desktopMain.includes("ipcMain.handle('account:retrySync'")
    && desktopMain.includes('forceAccountSync: true, allowProtectedStorage: true')
    && preloadScript.includes("retryAccountSync: (packKey = 'aht') => ipcRenderer.invoke('account:retrySync', { packKey })")
    && rendererApp.includes('const status = await window.aht.retryAccountSync(activeSidebarPack);')
    && rendererApp.includes('retry.textContent = "Retrying...";'),
  'An explicit account-sync retry must bypass cached status on every desktop platform, permit one user-requested protected-storage operation, and expose visible progress.'
);
...
assert(
  windowsInstallerInclude.includes('AHT includes Eclipse Temurin 8 (64-bit)')
  ...
  && desktopMain.includes('ensureBundledJava8'),
  'Windows ships and selects its bundled Java runtime without a separate Java installer choice.'
);
assert(
...
const minecraftBootstrapFlows = {
  ...
  update: desktopMain.slice(desktopMain.indexOf('function repairMinecraftRuntime'), desktopMain.indexOf('function useBundledJava8')),
...
assert(releaseWorkflow.includes('release-builds/phoenix-anticheat/Phoenix-Anti-cheat-Windows-x64-*.exe') && releaseWorkflow.includes('name: aht-launcher-phoenix-anticheat'), 'GitHub Windows builds must transport Phoenix as an independent publication artifact.');
...
assert(releaseWorkflow.includes('release_assets=(ci-artifacts/AHT-Launcher-Windows-10-11-*.exe ci-artifacts/*.dmg ci-artifacts/*.AppImage ci-launcher-update/launcher-latest.json)') && releaseWorkflow.includes('one-release Linux DEB compatibility bridge stay in R2') && !releaseWorkflow.includes('ci-artifacts/*.deb ci-artifacts/*.AppImage') && !releaseWorkflow.includes('release_assets=(ci-artifacts/Phoenix'), 'GitHub public releases must expose one Windows installer, one universal macOS DMG, and one Linux AppImage while keeping Phoenix and update-only artifacts out of the public asset list.');
assert(packagedGuardVerifier.includes('embeddedInLauncher: false') && packagedGuardVerifier.includes("fs.access(path.join(resources, 'native-guard'))") && windowsBuilderSource.includes('differentialPackage: false') && windowsBuilderSource.includes('useZip: true'), 'Packaged Windows verification must prove Phoenix is absent and the public installer uses the non-solid ZIP container.');
...
assert(smokePlayerUpdateLogs.includes('waitForNewsCarouselSettled') && smokePlayerUpdateLogs.includes("label, 40"), 'News carousel validation must allow a bounded ten-second transition cleanup window instead of assuming an exact runner timer.');
...
const playerLayoutForegroundIndex = smokePlayerLayout.indexOf("await client.call('Page.bringToFront')");
const playerLayoutHydrationIndex = smokePlayerLayout.indexOf("await waitFor(client, \"document.readyState === 'complete'");
assert(
  ...
  && smokePlayerLayout.includes("AHT_TEST_KEEP_RENDERER_ACTIVE: '1'")
  ...
  && smokePlayerLayout.includes("'--disable-background-timer-throttling'")
  && smokePlayerLayout.includes("'--disable-backgrounding-occluded-windows'")
  && smokePlayerLayout.includes("'--disable-renderer-backgrounding'")
...
assert(desktopMain.includes('requested_candidate="$requested_target.next-update"') && desktopMain.includes('backup_appimage="$target_appimage.previous-update"') && desktopMain.includes('nohup "$target_appimage"'), 'Linux AppImage updates must stage, back up, replace, and reopen the portable file.');
