...
assert(playServiceOutageSmoke.includes('/assets/1.12.json') && playServiceOutageSmoke.includes('Unexpected end of JSON input') && playServiceOutageSmoke.includes('Minecraft services') && playServiceOutageSmoke.includes('did not produce service-focused wording') && playServiceOutageSmoke.includes('AHT_TEST_ERROR_REPORT_CAPTURE_PATH') && playServiceOutageSmoke.includes('Copy full error details'), 'Minecraft service outage Play smoke must prove service wording, clickable diagnostics, and block raw JSON parse errors.');
...
assert(playAssetRepairSmoke.includes('AHT_TEST_MINECRAFT_ASSET_BASE_URL') && playAssetRepairSmoke.includes('Unable to prepare assets') && playAssetRepairSmoke.includes('asset-objects') && playAssetRepairSmoke.includes('assetObjectRequestCount !== 2') && playAssetRepairSmoke.includes('should be repaired before opening Minecraft Launcher') && playAssetRepairSmoke.includes('AHT_TEST_SPAWN_DETACHED_CAPTURE_PATH') && playAssetRepairSmoke.includes("['--workDir', mcRoot]") && playAssetRepairSmoke.includes('desktopMinecraftLauncher') && !playAssetRepairSmoke.includes('root-owned Minecraft Launcher executable') && playAssetRepairSmoke.includes('windowsHide === true') && playAssetRepairSmoke.includes('DISABLE_RTSS_LAYER'), 'Minecraft asset repair Play smoke must prove AHT repairs the asset index and missing/corrupt asset objects before opening a real Minecraft Launcher visibly with --workDir.');
...
assert(packageScripts['test:play-app-alias-fallback'] === 'node scripts/smoke-play-curseforge-priority.mjs --no-desktop --app-alias-start-fallback' && verifyLocalScript.includes("['test:play-app-alias-fallback']") && verifyInstalledScript.includes("['test:play-app-alias-fallback']"), 'Windows app-alias start fallback Play smoke must be wired into local and installed-player verification.');
...
assert(minecraftLauncherRoutesTest.includes('documents-curseforge-root') && minecraftLauncherRoutesTest.includes('macos-library-curseforge-root'), 'Route planner tests must cover alternate Windows and macOS CurseForge install locations, not only this host path.');
assert(minecraftLauncherRoutesSource.includes('windowsMinecraftLauncherDriveRoots') && minecraftLauncherRoutesSource.includes("'XboxGames', 'Minecraft Launcher', 'Content', 'Minecraft.exe'") && minecraftLauncherRoutesSource.includes("'Microsoft', 'WindowsApps', 'MinecraftLauncher.exe'") && minecraftLauncherRoutesTest.includes('xbox-games-launcher') && minecraftLauncherRoutesTest.includes('windows-app-alias-launcher'), 'Windows route planner must cover official Xbox/Game Pass installs and Windows app-execution aliases without opening CurseForge.');
assert(minecraftLauncherRoutesSource.includes('WINDOWS_MINECRAFT_LAUNCHER_SOURCE_PRIORITY') && minecraftLauncherRoutesSource.includes("'windows-app-alias': 90") && minecraftLauncherRoutesSource.includes('.sort((a, b) => a.priority - b.priority') && minecraftLauncherRoutesTest.includes('shortcut-before-windows-app-alias'), 'Windows launcher executable candidates must use explicit source priority so stronger shortcut/desktop routes beat weaker app aliases.');
...
assert(desktopMain.includes('async function minecraftLauncherOpenRoutes(config, env)') && desktopMain.includes('async function openMinecraftLauncherRoute(route, cwd, env)') && desktopMain.includes('planMacMinecraftLauncherRoutes({'), 'Play must use the shared Minecraft Launcher route planner/executor for macOS and Windows.');
assert(desktopMain.includes('await customMinecraftLauncherRouteIsAvailable(customRoute)') && desktopMain.includes('return [...routes, customRoute];') && !desktopMain.includes('if (config.minecraftLauncher?.openCommand) {\n    return { ...await spawnDetached'), 'Custom Minecraft launcher commands must be available fallback routes, not a CurseForge-first short-circuit.');
...
assert(minecraftLauncherRoutesTest.includes('macos-curseforge-app-path') && minecraftLauncherRoutesTest.includes('macos-curseforge-bundle-fallback') && minecraftLauncherRoutesTest.includes('macos-custom-app') && minecraftLauncherRoutesTest.includes("'--args', '--workDir'"), 'macOS route planner tests must prove CurseForge root preference and --workDir handoff.');
...
assert(desktopMain.includes("route.kind === 'store'") && desktopMain.includes('return openWindowsStoreMinecraftLauncher(cwd, env);'), 'Windows play fallback must use the robust Store opener through the shared route executor.');
...
assert((desktopMain.match(/ensureAssetObjects: true/g) || []).length >= 2 && (desktopMain.match(/verifyAssetHashes: true/g) || []).length >= 2, 'Modpack Update finalization and Play must repair missing/corrupt Minecraft asset objects before handing off to Minecraft Launcher.');
