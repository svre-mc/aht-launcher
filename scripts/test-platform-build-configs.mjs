...
const smokeCloseDuringUpdate = fs.readFileSync(new URL('../scripts/smoke-close-during-update.mjs', import.meta.url), 'utf8');
...
const electronSmokeIsolationFailures = fs.readdirSync(new URL('./', import.meta.url))
  ...
  .map((name) => ({ name, source: fs.readFileSync(new URL(name, import.meta.url), 'utf8') }))
  .filter(({ source }) => source.includes('--user-data-dir') && (
...
assert(rendererHtml.includes('<span>Date</span><span>IPv4</span><span>Platform</span><span>Version</span><span>File</span>') && rendererHtml.includes('<span>Registered</span><span>User</span><span>IPv4</span><span>MC UUID</span><span>Platform</span>') && rendererApp.includes('if (platform.includes("win")) return "Windows"') && rendererApp.includes('return "Mac";'), 'Player Data must keep anonymous installer downloads separate from professional verified-player fields and use short platform names.');
...
assert(rendererApp.includes('runFriendAction("add_friend")') && rendererApp.includes('"remove_friend", "Unadd"') && rendererApp.includes('"unblock_player", "Unblock"') && rendererApp.includes('"block_player"'), 'Renderer must expose Add, Unadd, Block, and Unblock actions.');
assert(rendererHtml.includes('id="blockPlayerButton"') && rendererHtml.includes('id="profileFriendsBadge"') && rendererHtml.includes('id="friendsRequestsList"'), 'Friends UI must expose launcher block, request list, and unread badge controls.');
...
assert(socialClientSource.includes("'block_player'") && socialClientSource.includes("'accept_friend'") && socialClientSource.includes("'decline_friend'"), 'Social client action allowlist must include launcher friend and block actions.');
assert(workerSource.includes("'block_player'") && workerSource.includes("'/server/social/sync'") && workerSource.includes("'/api/social/actions'"), 'Worker must enforce the expanded social action allowlist and signed server sync route.');
assert(workerSource.includes('Social sync must come from the Linux AHT server.') && workerSource.includes("env.AHT_SOCIAL_SERVER_ID || 'aht-linux'"), 'Worker must restrict social sync to the Linux AHT server identity.');
assert(friendsPanelSmoke.includes("actionNames !== 'add_friend,remove_friend,unblock_player,accept_friend,block_player'") && friendsPanelSmoke.includes("Object.keys(entry).sort().join(',') !== 'action,target'"), 'Friends panel smoke must prove launcher friend/block actions and prevent renderer identity payloads.');
...
assert(checkProductionReadiness.includes('api/launcher-proof/status') && checkProductionReadiness.includes('json.dedicatedConfigured === true') && checkProductionReadiness.includes('json.signingVerified === true') && workerSource.includes('const dedicatedConfigured = Boolean(env.LAUNCHER_PROOF_SECRET || env.AHT_LAUNCHER_PROOF_SECRET)') && !checkProductionReadiness.includes('AHTProofCheck') && !checkProductionReadiness.includes('api/users/register'), 'Production readiness must require the Worker read-only dedicated launcher-proof signing self-test without creating synthetic player records.');
