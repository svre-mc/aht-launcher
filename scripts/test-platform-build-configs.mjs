...
assert(smokeCloseDuringUpdate.includes("javaPath: fakeJavaPath") && smokeCloseDuringUpdate.includes("AHT_TEST_JAVA_RUNTIME_PROBE: 'release-file'") && smokeCloseDuringUpdate.includes("AHT_MINECRAFT_MAC_APP: process.platform === 'darwin' ? macMinecraftApp : ''") && smokeCloseDuringUpdate.includes('clearTimeout(timer);') && smokeCloseDuringUpdate.includes("String(target.title || '').trim() === 'A Hard Time Launcher'") && smokeCloseDuringUpdate.includes('async function connectReadyLauncherPage()') && smokeCloseDuringUpdate.includes("document.body.classList.contains('is-launcher-ready')") && smokeCloseDuringUpdate.includes('CDP socket open timed out') && smokeCloseDuringUpdate.includes('closeAllConnections') && smokeCloseDuringUpdate.includes('Pack response stayed open after the launcher exited.'), 'Close-during-update validation must isolate host prerequisites, wait for the stable launcher-ready page, and hard-bound every debugger and HTTP cleanup wait.');
