...
assert(playAssetRepairSmoke.includes('AHT_TEST_MINECRAFT_ASSET_BASE_URL') && playAssetRepairSmoke.includes('Unable to prepare assets') && playAssetRepairSmoke.includes('asset-objects') && playAssetRepairSmoke.includes('assetObjectRequestCount !== 0') && playAssetRepairSmoke.includes('should not be downloaded during Play') && playAssetRepairSmoke.includes('AHT_TEST_SPAWN_DETACHED_CAPTURE_PATH') && playAssetRepairSmoke.includes("['--workDir', mcRoot]") && playAssetRepairSmoke.includes('root-owned Minecraft Launcher executable') && playAssetRepairSmoke.includes('windowsHide === true') && playAssetRepairSmoke.includes('DISABLE_RTSS_LAYER'), 'Minecraft asset repair Play smoke must prove AHT repairs the asset index, avoids raw asset errors, skips full asset-object downloads, and opens the root-owned Minecraft Launcher visibly with --workDir.');
...
assert((desktopMain.match(/ensureAssetObjects: false/g) || []).length >= 2, 'Modpack Update finalization and Play must avoid full Minecraft asset-object downloads; setup/repair owns heavier repair.');
