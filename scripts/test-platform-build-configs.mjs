...
assert(playStartSource.indexOf("'prepared-prerequisites'") < playStartSource.indexOf("'protected-client-integrity'") && playStartSource.indexOf("'protected-client-integrity'") < playStartSource.indexOf("'phoenix-anticheat'") && playStartSource.indexOf("'phoenix-anticheat'") < playStartSource.indexOf("'prepared-play-attestation'") && playStartSource.indexOf("'prepared-play-attestation'") < playStartSource.indexOf('const launcherOpening = (') && playStartSource.includes('managedFilesChecked: finalManagedIntegrity.managedFilesChecked') && playStartSource.includes('runtimeFilesChecked: 0') && preparedPlayIntegritySource.includes('verifyManagedIntegritySnapshot') && preparedPlayIntegritySource.includes('forceAll: !trusted.complete && !previousFileStates.length') && preparedPlayIntegritySource.includes('let previousFileStates = trusted.complete ? trusted.fileStates : []') && preparedPlayIntegritySource.includes('matchingManagedFileStates(prepared.previousManagedSnapshot, managedFiles)') && playStartSource.includes('if (prepared.minecraftProfile?.selectionPrepared !== true)') && playStartSource.includes('selectPreparedMinecraftLauncherProfile(prepared.minecraftProfile)') && playStartSource.includes('await persistPreparedLaunchEntry(key, prepared)') && !playStartSource.includes('verifyPreparedRuntimeSnapshot') && playStartSource.includes('proof = await refreshPreparedLauncherProof(key, prepared, nativeGuard, { allowInteractiveRecovery: true });'), 'Play must verify protected files, Phoenix and authorization before handoff, hash only metadata changes after its signed baseline, and retain the fast saved launcher/runtime path.');
