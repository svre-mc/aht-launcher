...
assert(rendererApp.includes('if (initialState.initialized && !initialState.firstInitialization)') && rendererApp.includes('preparation = await window.aht.prepareStartup();') && rendererApp.indexOf('preparation = await window.aht.prepareStartup();') < rendererApp.indexOf('newsStatusResults = await loadNewsStatusResults(true);') && desktopMain.includes("const usePreparedPrerequisites = prepared?.state === 'ready';") && desktopMain.includes('? prepared.launcherConfig') && desktopMain.includes('identityForStatus(launcherConfig, usePreparedPrerequisites ? prepared : null, allowProtectedStorage)'), 'Warm startup must hydrate its trusted prerequisite entries before status rendering and reuse their launcher route while refreshing the signed-in account instead of repeating launcher discovery.');
...
assert(
  publicDeviceIdentitySource.includes('readJsonFile(deviceIdentityPath())')
    ...
    && getStatusSource.includes('identityForStatus(launcherConfig, usePreparedPrerequisites ? prepared : null, allowProtectedStorage)')
    && desktopMain.includes('const identity = await identityPayload(launcherConfig, { allowProtectedStorage });')
