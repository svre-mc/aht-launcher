...
assert(desktopMain.includes("openMacMinecraftLauncher(config, cwd, env)") && desktopMain.includes('planMacMinecraftLauncherRoutes({'), 'macOS play must use the shared macOS Minecraft Launcher route planner.');
...
assert(desktopMain.includes("route.kind === 'store'") && desktopMain.includes('openWindowsStoreMinecraftLauncher(route.cwd || cwd, env)'), 'Windows play fallback must use the robust Store opener.');
