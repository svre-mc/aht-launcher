...
const electronSmokeIsolationFailures = fs.readdirSync(new URL('./', import.meta.url))
  ...
  .filter(({ source }) => source.includes('--user-data-dir') && !source.includes('PACKAGED_PRODUCTION_UPDATE_SMOKE') && (
    ...
    || (!source.includes('AHT_TEST_USER_DATA: userData') && !source.includes('AHT_TEST_USER_DATA: developerUserData'))
  ))
  .map(({ name }) => name);
...
assert(rendererApp.includes('function buildNewsFeatureCarousel') && rendererApp.includes('function selectNewsCarouselSlide') && rendererApp.includes('function playNewsCarouselMedia') && rendererApp.includes('function transitionNewsSurface') && rendererHtml.includes('class="news-transition-loader"') && rendererHtml.includes('id="updateLogInlineMedia"') && rendererPolishCss.includes('.news-carousel-caption') && rendererPolishCss.includes('filter: brightness(1.1);') && rendererPolishCss.includes('.news-feed-card:active') && rendererPolishCss.includes('.update-log-article-footer {') && rendererPolishCss.includes('.news-view.is-transitioning:not(.is-transition-entering) .news-transition-loader {\n  opacity: 0.92;\n  transition: none;'), 'News must preserve the measured BSG carousel, inline-media, immediately visible transition-loader pocket, thumbnail-brightness, and static press-state interaction contract.');
...
assert(desktopMain.includes('function useUnencryptedDeviceSecretTestFallback()') && desktopMain.includes("process.env.AHT_TEST_HOOKS === '1'") && desktopMain.includes('protectDeviceSecret(created.privateKey)') && verifyLocalScript.includes("AHT_ALLOW_UNENCRYPTED_DEVICE_KEY: '1'") && verifyInstalledPlayerScript.includes("AHT_ALLOW_UNENCRYPTED_DEVICE_KEY: '1'") && releaseWorkflow.includes('AHT_ALLOW_UNENCRYPTED_DEVICE_KEY: "1"'), 'Headless Electron validation must bypass macOS secure storage only behind explicit test hooks while production device credentials remain OS-encrypted.');
