...
assert(errorDetailsCopySmoke.includes('AHT_TEST_ERROR_REPORT_CAPTURE_PATH') && errorDetailsCopySmoke.includes('Copy full error details') && errorDetailsCopySmoke.includes('diagnostic-smoke'), 'Error-details copy smoke must prove the clickable copy action writes the full diagnostic report.');
...
assert(playAssetRepairSmoke.includes('AHT_TEST_MINECRAFT_ASSET_BASE_URL') && playAssetRepairSmoke.includes('Unable to prepare assets') && playAssetRepairSmoke.includes('asset-objects') && playAssetRepairSmoke.includes('bad first asset response') && playAssetRepairSmoke.includes('assetObjectRequestCount < 2') && playAssetRepairSmoke.includes('fake Minecraft Launcher marker'), 'Minecraft asset repair Play smoke must prove AHT retries bad asset objects and repairs index/object before opening Minecraft Launcher.');
...
assert(desktopMain.includes('ensureAssetObjects: false'), 'Modpack Update finalization must avoid full Minecraft asset-object downloads; Play preflight owns that heavier repair.');
