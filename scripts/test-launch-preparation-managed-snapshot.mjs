...
try {
  ...
  const physicalStatsByPath = new Map();
  const unchanged = await verifyManagedIntegritySnapshot(root, {
    managedFiles,
    ignoreLocalManaged: true,
    previousFileStates: initial.fileStates,
    legacySince: cachedAt,
    onFingerprintPhysicalStat: (relativePath) => {
      physicalStatsByPath.set(relativePath, (physicalStatsByPath.get(relativePath) || 0) + 1);
    }
  });
  assert(unchanged.valid, `Unchanged launch-critical files failed warm verification: ${JSON.stringify(unchanged.issues)}`);
  assert.equal(unchanged.hashedFiles, 0, 'Unchanged launch-critical files were rehashed.');
  for (const item of critical) {
    assert.equal(
      physicalStatsByPath.get(item.relativePath),
      1,
      `Warm verification performed a duplicate physical metadata read for ${item.relativePath}.`
    );
  }

  await fs.mkdir(path.join(root, 'resources', 'contentcreator', 'models', 'item'), { recursive: true });
  const emptyRuntimeDirectories = await verifyManagedIntegritySnapshot(root, {
    managedFiles,
    ignoreLocalManaged: true,
    previousFileStates: initial.fileStates,
    legacySince: cachedAt
  });
  assert(emptyRuntimeDirectories.valid, `Empty runtime-created directories invalidated preparation: ${JSON.stringify(emptyRuntimeDirectories.issues)}`);
  assert.equal(emptyRuntimeDirectories.hashedFiles, 0, 'Empty directory scaffolding caused managed files to be rehashed.');

  const unmanagedNestedResource = 'resources/contentcreator/models/item/injected.json';
  await write(unmanagedNestedResource, '{"untrusted":true}');
  const nestedResourcePayload = await verifyManagedIntegritySnapshot(root, {
    managedFiles,
    ignoreLocalManaged: true,
    previousFileStates: initial.fileStates,
  ...
  assert(!nestedResourcePayload.valid, 'An unmanaged file inside runtime-created directory scaffolding was trusted.');
  assert(nestedResourcePayload.issues.some((item) => item.reason === 'managed-path-set-changed'));
  await fs.rm(path.join(root, ...unmanagedNestedResource.split('/')), { force: true });
  ...
  console.log(JSON.stringify({
    ...
    onePassWarmMetadataVerification: true,
