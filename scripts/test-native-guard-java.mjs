...
const guard=spawn(path.join(root,'build/native-guard/AHT Runtime Guard.exe'),[],{windowsHide:true,stdio:['pipe','pipe','pipe']});
try {
 const pid=Number(await firstLine(java));const ready=firstLine(guard);guard.stdin.end(JSON.stringify({gameDir:out,javaPath:path.join(javaHome,'bin/java.exe')})+'\n');const info=JSON.parse(await ready);
 await new Promise(r=>setTimeout(r,2600));await request(info.port,pid);const samples=[];
 for(let i=0;i<3;i++){await new Promise(r=>setTimeout(r,2400));const fields=await request(info.port,pid);samples.push({state:fields[7],modules:Number(fields[8]),bytes:Number(fields[9]),detail:Buffer.from(fields[10],'base64url').toString()});assert.equal(fields[7],'clean',JSON.stringify(samples.at(-1)));}
