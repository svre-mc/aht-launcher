...
2. Builds macOS Apple Silicon and Intel DMGs plus self-update ZIPs on a macOS runner.
...
5. Uploads public launcher installers and `launcher-latest.json` to that GitHub Release.
...
macOS ZIP files are generated by the workflow for in-app self-updates and uploaded through the R2 launcher update plan. They are not public manual-download assets.
